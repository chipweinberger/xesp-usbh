...
{
    HCD_CHECK(port_hdl != NULL && pipe_config != NULL && ctrl_pipe != NULL, ESP_ERR_INVALID_ARG);
    port_t *port = (port_t *)port_hdl;
    HCD_ENTER_CRITICAL();
    ...
    port->num_pipes_idle++;
    HCD_EXIT_CRITICAL();
    *ctrl_pipe = (hcd_pipe_handle_t)pipe;
    return ret;
