...
static bool pipe_callback(hcd_pipe_handle_t ctrl_pipe, hcd_pipe_event_t pipe_event, void *user_arg, bool in_isr)
{
...
static void expect_pipe_event(QueueHandle_t pipe_evt_queue, hcd_pipe_handle_t expected_hdl, hcd_pipe_event_t expected_event)
{
    ...
    TEST_ASSERT_EQUAL(expected_hdl, msg.ctrl_pipe);
                                     ...
                                     int num_xfers)
{
    ...
    TEST_ASSERT_EQUAL(ESP_OK, hcd_pipe_alloc(port_hdl, &config, ctrl_pipe));
    TEST_ASSERT_NOT_EQUAL(NULL, *ctrl_pipe);
    ...
    for (int i = 0; i < num_xfers; i++) {
        ...
        hcd_xfer_req_set_target(req_hdls[i], *ctrl_pipe, irps[i], NULL);
    }
}

static void free_pipe_and_xfer_reqs(hcd_pipe_handle_t ctrl_pipe,
                                    hcd_xfer_req_handle_t *req_hdls,
                                    uint8_t **data_buffers,
                                    usb_irp_t **irps,
                                    int num_xfers)
{
    ...
    //Delete the pipe
    TEST_ASSERT_EQUAL(ESP_OK, hcd_pipe_free(ctrl_pipe));
}
...
TEST_CASE("Test HCD port sudden disconnect", "[hcd][ignore]")
{
    ...
    for (int i = 0; i < NUM_XFER_REQS; i++) {
        ...
        usb_irp_t *irp;
        void *context;
        hcd_xfer_req_get_target(req_hdl, &ctrl_pipe, &irp, &context);
        TEST_ASSERT_EQUAL(default_pipe, ctrl_pipe);
